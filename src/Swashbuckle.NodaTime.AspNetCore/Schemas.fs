...
module internal Schemas =
  ...
  type SchemaCreator(?serializerSettings : JsonSerializerSettings) =
    ...
    member __.Create() =
      ...
      let interval = Interval(instant, instant
      ...
      { Container.Instant = __.StringSchema(instant, "date-time");
        ...
        Container.LocalTime = __.StringSchema(zonedDateTime.TimeOfDay, "partial-time");
        Container.LocalDateTime = __.StringSchema zonedDateTime.LocalDateTime;
        Container.OffsetDateTime = __.StringSchema(instant.WithOffset zonedDateTime.Offset, "date-time");
        Container.ZonedDateTime = __.StringSchema zonedDateTime;
        Container.Interval = OpenApiSchema(Type = "object", Properties = dict
          [("Start", __.StringSchema(interval.Start, "date-time"));
          ("End", __.StringSchema(interval.End, "date-time"))]);
        Container.Offset = __.StringSchema(zonedDateTime.Offset, "time-numoffset");
        Container.Period = __.StringSchema(Period.Between
